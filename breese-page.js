class BreesePage extends HTMLElement {
  constructor() {
    super();
  ...
  setupScrollObserver() {
    ...
    const options = {
      root: this.shadowRoot.querySelector('.content'),
      threshold: 0.5
    };
    ...
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          this.setAttribute('active', entry.target.id);
        }
      });
    }, options);
  ...
  render() {
      ...
      <div class="page">
        <nav class="nav-container">
          ...
          <div class="nav">
            ${links.map(link => `
              <h1 
                class="nav-link ${link.id === 'home' ? 'active' : ''}" 
                data-anchor="${link.id}"
              >
                ${link.label}
              </h1>
    ...
    this.shadowRoot.querySelectorAll('.nav-link').forEach(link => {
      link.addEventListener('click', () => {
        this.scrollToSection(link.dataset.anchor);
      });
    });
  }
}

customElements.define('breese-page', BreesePage);